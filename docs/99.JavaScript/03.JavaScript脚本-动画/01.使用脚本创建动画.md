...
1. **`rotation`**  
   - 值：`0` → `360`  
   - 含义：元素旋转角度，从 0 度到 360 度，通常表示顺时针完整旋转一圈。

2. **`scaleX`**  
   - 值：`1` → `0.2`  
   - 含义：X 轴（水平方向）的缩放比例，从原始大小缩放到 0.2 倍宽（即宽度缩小为 20%）。

3. **`scaleY`**  
   - 值：`1` → `0.2`  
   - 含义：Y 轴（垂直方向）的缩放比例，从原始大小缩放到 0.2 倍高。

4. **`translateX`**  
   - 值：`0` → `60`  
   - 含义：X 轴方向平移，从原位向右移动 60 单位（可能是像素、点等）。

5. **`translateY`**  
   - 值：`0` → `-180`  
   - 含义：Y 轴方向平移，从原位向上移动 180 单位（因为一般是向下为正，向上为负）。

6. **`opacity`**  
   - 值：`1` → `0`  
   - 含义：透明度，从完全不透明（1）变为完全透明（0），即淡出消失。

7. **`rotationY`**
   - 值类型：浮点数
   - 值范围示例：`0, 180`
   - 含义：绕Y轴（垂直轴）旋转，从0度到180度，产生水平翻转的3D效果

8. **`rotationX`**
   - 值类型：浮点数
   - 值范围示例：`0, 90`
   - 含义：绕X轴（水平轴）旋转，从0度到90度，产生垂直翻转的3D效果
